...
parser = argparse.ArgumentParser(description='An Event Driven Molecular Dynamics (EDMD) Simulator - Sah-Vi Simulation',
                                 formatter_class=argparse.ArgumentDefaultsHelpFormatter)
#número de partículas
parser.add_argument('-n', action="store", dest="npart", default=50, type=int,
...
def simulate(options, particles, time):
    ...
    # Computa o tempo da próxima colisão
...
def initParticles(n,r,xmax, ymax, color):
    ...
    for i in range(int(round(xmax/dx))):
        for j in range(int(round(ymax/dx))):
            if (n_init < n):
                parts.append(Particle(dx*(i+0.5),dx*(j+0.5),r, color))
            n_init += 1
    return parts
